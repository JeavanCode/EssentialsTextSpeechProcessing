...
plt.plot(merged_data['Year'], merged_data['Positive'], label='Positive Emotion Score')
plt.plot(merged_data['Year'], merged_data['Negative'], label='Negative Emotion Score')
plt.plot(merged_data['Year'], merged_data['Overall'], label='Overall Emotion Score')
plt.xlabel("Year")
plt.ylabel("Emotion Score")
plt.title("Emotion Scores by Year (2010-2017)")
plt.legend()
plt.grid(True)
plt.savefig('./Figures/emotion_scores_2010_2017.png')
plt.show()

# Mental Health Trends vs Emotion Scores (2010-2017)
for column in merged_data.columns[4:]:
    plt.figure(figsize=(12, 6))
    plt.plot(merged_data['Year'], merged_data[column], label=column, color='purple')
    plt.plot(merged_data['Year'], merged_data['Negative'], label='Negative Emotion Score', color='red')
    plt.xlabel("Year")
    plt.ylabel("Rate")
    plt.title(f"{column} vs Negative Emotion Score (2010-2017)")
    plt.legend()
    plt.grid(True)
    plt.savefig(f'./Figures/{column}_vs_negative_emotion_2010_2017.png')
    plt.show()
